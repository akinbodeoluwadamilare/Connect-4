...
public class GameControllerTest {
    ...
    public void testInvalidMove_InvalidPlayerID() {
        ...
        // Mock the behavior of the GameModel
        GameModel gameModelMock = mock(GameModel.class);
        when(gameModelMock.makeMove(anyInt(), anyInt(), anyInt())).thenThrow(new Exception("Invalid Player ID.")); // Simulate exception for invalid player ID
        gameController.setGameModel(gameModelMock); // Set the mock GameModel to the GameController
        ...
        // Assert
        assertFalse(response.isSuccess()); // Check if the response indicates failure
        assertEquals("Invalid Player ID.", response.getErrorMessage()); // Check if the error message is "Invalid Player ID."
        assertEquals(0, response.getGameId()); // Check if the game ID is 0
    ...
    public void testInvalidMove_InvalidColumn() {
        ...
        // Mock the behavior of the GameModel
        GameModel gameModelMock = mock(GameModel.class);
        when(gameModelMock.makeMove(anyInt(), anyInt(), anyInt())).thenThrow(new Exception("Invalid Column.")); // Simulate exception for invalid column
        gameController.setGameModel(gameModelMock); // Set the mock GameModel to the GameController
    ...
    public void testInvalidPlayerTurn() {
        // Arrange
        PlayGameRequest request = new PlayGameRequest(1, 1, 2); // Prepare the request
        
        // Mock the behavior of the GameModel
        GameModel gameModelMock = mock(GameModel.class);
        when(gameModelMock.makeMove(anyInt(), anyInt(), anyInt())).thenThrow(new Exception("Cannot make move.")); // Simulate exception for invalid player turn
        gameController.setGameModel(gameModelMock); // Set the mock GameModel to the GameController
        ...
        assertEquals(0, response.getPlayer1Id()); // Check if player1 ID is 0
        assertEquals(0, response.getPlayer2Id()); // Check if player2 ID is 0
        assertEquals(2, response.getCurrentTurnPlayer()); // Check if current turn player is 2
        assertEquals("Playing", response.getGameStatus()); // Check if the game status is "Playing"
        assertEquals("000000000000000000000000000000000000000000", response.getGameBoard()); // Check if the game board is as expected
    }
    ...
    public void testCannotMakeMoveInCompletedGame() {
        ...
        // Mock the behavior of the GameModel
        GameModel gameModelMock = mock(GameModel.class);
        when(gameModelMock.makeMove(anyInt(), anyInt(), anyInt())).thenThrow(new Exception("Cannot make move.")); // Simulate exception for completed game
        gameController.setGameModel(gameModelMock); // Set the mock GameModel to the GameController
    ...
    @Test
    public void testCannotMakeMoveInFullColumn() {
        // Arrange
        PlayGameRequest request = new PlayGameRequest(1, 1, 5); // Prepare the request
        
        // Mock the behavior of the GameModel
        GameModel gameModelMock = mock(GameModel.class);
        when(gameModelMock.makeMove(anyInt(), anyInt(), eq(5))).thenThrow(new Exception("Cannot make move.")); // Simulate exception for full column
        gameController.setGameModel(gameModelMock); // Set the mock GameModel to the GameController
        ...
        assertEquals("Playing", response.getGameStatus()); // Check if the game status is "Playing"
        assertEquals("1212121212121212121212121", response.getGameBoard()); // Check if the game board is as expected
    }
}
